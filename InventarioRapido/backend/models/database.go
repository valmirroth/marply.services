...
func (d *Database) BuscarContagemAtiva(empresa string) (string, error) {
	...
	const insertContagem = `
		INSERT INTO CST_REGISTRO_CONTAGEM_LOCAL (CODIGO, STATUS)
		SELECT ISNULL(MAX(ISNULL(CODIGO,0)),0) + 1, 'ATIVO'
...
func (d *Database) InserirContagem(contagem ContagemRequest) (*ContagemLocal, error) {
	...
	const query = `
		INSERT INTO CST_CONTAGEM_LOCAL 
			(CODIGO_ITEM, DESCRICAO_ITEM, LOCAL, QUANTIDADE, VOLUMES, USUARIO_CONTAGEM, SALDO_ESTOQUE, NROCONTAGEM, CODLOCAL)
	...
	nrocontagem, errocontagem := d.BuscarContagemAtiva("")
	if errocontagem != nil {
	...
	xlocal, erro := d.ValidarLocal(contagem.Local)
	if erro != nil {
