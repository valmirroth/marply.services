...
const formSchema = insertInventoryCountSchema.extend({
  ...
  volumeCount: z.string().min(1, "Volume é obrigatório"),
});
...
export default function InventoryForm({ onSuccess, disabled = false }: InventoryFormProps) {
  ...
  const buscarDescricaoItem = async (codigo: string) => {
    ...
    try {
      ...
      const response = await fetch(`http://192.168.1.28:9090/api/items/${codigo}/description`);
  ...
  const validarLocal = async (sigla: string) => {
    ...
    try {
      const response = await fetch(`http://192.168.1.28:9090/api/locations/${sigla}/validate`);
  ...
  const validarEmbalagem = useCallback(() => {
    ...
    const volumes = parseInt(form.getValues("volumeCount")) || 0;
  ...
  const onSubmit = (data: FormData) => {
    const submitData: InsertInventoryCount = {
      ...
      volumeCount: parseInt(data.volumeCount),
    };
