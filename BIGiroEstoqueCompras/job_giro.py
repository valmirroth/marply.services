    ...
    try:
        ...
        query = """ declare @di varchar(10)
            ...
            from ESTOQUE_POSICAO 
                ...
                WHERE   EMPRESA_RECNO = 1 --and ITEM = '000292'
                and ESTOQUE_POSICAO.DATAHORA = '20241231'
                and  ESTOQUE_POSICAO.SALDOREAL > 0
               AND ESTOQUE_POSICAO.ITEM NOT IN (select distinct CODIGO FROM HISREAL WHERE DTRECEB >= @di  and DTRECEB <= @df AND EMPRESA_RECNO = 1 AND MOV_EFETIVA = 'S')
    UNION ALL 
            ...
            from HISREAL 
               ...
               INNER JOIN ESTOQUE ON ESTOQUE.CODIGO = HISREAL.CODIGO  and ESTOQUE.CATEGORIA not in ( '99','10','110')
                ...
                and FORMA in  ('e','s')  
                                ...
                                where  EMPRESA_RECNO = 1 AND DTRECEB <= @df
        ...
        cursor = conn.cursor()
        delete_sql = "DELETE FROM CST_AnaliseGiroEstoque where empresarecno = '1' "
